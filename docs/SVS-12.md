...
SVS-12 is validated locally against a clean `solana-test-validator` with the
program preloaded under `6HExmbtZ1WnRC3QyphvtDNFpXgKqUz66UG4YoAi1beT8`.

- Program + SDK tests: `68 passing`
- E2E scripts: `basic`, `multi-user`, `edge-cases`, `admin`, `loss-rebalance`, `inflation-attack`
- CLI smoke: `tranched initialize`, `tranched add-tranche`, and `tranched info --output json`

Current status: local validation complete, devnet deployment still pending.
