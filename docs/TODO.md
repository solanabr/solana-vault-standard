...
- ~~**Extract attestation parsing into a reusable crate**~~ — DONE. The canonical layout + `Attestation::from_account_data` / `check_invariants` / `verify_attestation` now live in `modules/svs-attestation`, consumed by SVS-11 (`validate_attestation`), `compliance-hook` (`check_attestation`), and `derwa-wrapper` (`validate_investor_attestation`); each maps the granular `AttestationError` onto its own error codes. Mirrors the oracle extraction (`SvsOraclePrice` + `read_oracle` in `modules/svs-oracle`).
