...
- **Extract oracle / attestation parsing into reusable crates** — `svs-11/src/oracle.rs` and `svs-11/src/attestation.rs` manually skip 8 bytes and deserialize. Move to `modules/svs-oracle` and a new `modules/svs-attestation` crate so svs-8, svs-10, and future programs can share the code.
