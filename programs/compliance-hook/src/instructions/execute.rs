...
pub fn handler(ctx: Context<Execute>) -> Result<()> {
    ...
    match ctx.accounts.mint_config.mode {
        ...
        ComplianceMode::Permissioned => {
            ...
            // Validate both endpoints against the mint's trust anchors.
            ...
            // pool_policy thresholds (jurisdiction / investor_class /
            // kyc_risk_tier) are wired but not yet enforced; the EAML
            // resolves the account so a future upgrade can flip the
...
/// Read an SVS-11-shaped Attestation account and validate it against the
/// mint's configured trust anchors.
///
...
/// Must stay in sync with `programs/svs-11/src/attestation.rs` and the
/// parallel reader in `programs/derwa-wrapper/src/instructions/unwrap.rs`.
fn check_attestation(
    ...
    // 1. Owner: must be the configured attestation program.
    require!(
    ...
    // 3. Subject: must match the ATA owner.
    let subject_bytes: [u8; 32] = payload[0..32]
        .try_into()
        .map_err(|_| error!(ComplianceHookError::AttestationNotFound))?;
    ...
    // 4. Issuer: must match the mint-configured issuer.
    ...
    // 5. Attestation type: must match the mint-required type.
    ...
    // 8. Canonical PDA derivation. Atomically binds 3-5 to the same
    //    physical account.
    let bump = payload[84];
