...
pub struct Initialize<'info> {
    ...
    #[account(
        ...
        bump,
        token::mint = shares_mint_interface,
        token::authority = vault,
        token::token_program = token_2022_program,
    )]
    pub share_escrow: InterfaceAccount<'info, TokenAccount>,

    /// CHECK: used only for share_escrow init
    pub shares_mint_interface: UncheckedAccount<'info>,
...
pub fn handler(
    ...
    let shares_seeds: &[&[u8]] = &[SHARES_MINT_SEED, vault_key.as_ref(), &[shares_mint_bump]];
    ...
    invoke_signed(&init_mint_ix, &[ctx.accounts.shares_mint.to_account_info()], &[shares_seeds])?;
