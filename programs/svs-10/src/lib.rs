...
declare_id!("E6gqyoVDQ33cWFJ9LpdSu68fNw6EKmoKR4db288RpFgJ");
...
pub mod svs_10 {
    ...
    pub fn initialize(
        ...
        instructions::initialize::handler(ctx, vault_id, cancel_delay, max_staleness)
    }
    ...
    pub fn revoke_operator(ctx: Context<RevokeOperator>) -> Result<()> {
        instructions::revoke_operator::handler(ctx)
    }
    ...
    pub fn initialize_oracle(
        ...
        instructions::admin::initialize_oracle(ctx, initial_price, oracle_authority)
    }

    pub fn update_oracle_price(ctx: Context<UpdateOracle>, new_price: u64) -> Result<()> {
        instructions::admin::update_oracle_price(ctx, new_price)
    }
