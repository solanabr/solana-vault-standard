...
use crate::{
    ...
    error::VaultError,
    events::*,
    state::{CreditVault, RedeemRequest, RequestStatus},
};
...
pub fn request_redeem(ctx: Context<RequestRedeem>, shares: u64, receiver: Pubkey) -> Result<()> {
    ...
    emit!(RedeemRequested { vault: ctx.accounts.vault.key(), owner: ctx.accounts.owner.key(), shares });
    Ok(())
}
...
pub fn approve_redeem(ctx: Context<ApproveRedeem>) -> Result<()> {
    ...
    ctx.accounts.redeem_request.status = RequestStatus::Approved;
    ...
    emit!(RedeemApproved { vault: ctx.accounts.vault.key(), owner, shares, assets });
    Ok(())
}
...
pub fn cancel_redeem(ctx: Context<CancelRedeem>) -> Result<()> {
    ...
    emit!(RedeemCancelled { vault: ctx.accounts.vault.key(), owner: ctx.accounts.owner.key(), shares_returned: shares });
    Ok(())
}
...
pub struct ClaimRedemption<'info> {
    ...
    #[account(
        mut,
        seeds = [REDEEM_REQUEST_SEED, vault.key().as_ref(), redeem_owner.key().as_ref()],
        bump = redeem_request.bump,
        constraint = redeem_request.status == RequestStatus::Approved @ VaultError::RequestNotApproved,
        close = redeem_owner,
    )]
    pub redeem_request: Account<'info, RedeemRequest>,
    ...
    /// CHECK: rent
    #[account(mut)]
    pub redeem_owner: UncheckedAccount<'info>,

    #[account(
        mut,
        seeds = [CLAIMABLE_TOKENS_SEED, vault.key().as_ref(), redeem_owner.key().as_ref()],
        bump,
    )]
    pub claimable_tokens: InterfaceAccount<'info, TokenAccount>,
...
pub fn claim_redemption(ctx: Context<ClaimRedemption>) -> Result<()> {
    ...
    emit!(RedemptionClaimed { vault: ctx.accounts.vault.key(), owner, receiver, assets });
    Ok(())
}
