...
use crate::{
    constants::{MIN_DEPOSIT_AMOUNT, STREAM_VAULT_SEED},
    error::VaultError,
...
pub fn handler(ctx: Context<Deposit>, assets: u64, min_shares_out: u64) -> Result<()> {
    ...
    // SVS-5: Use time-interpolated balance instead of live asset_vault.amount
    let clock = Clock::get()?;
    ...
    // Update base_assets to reflect the newly deposited tokens.
    // The deposit increases the actual balance in asset_vault, which must be
    // reflected in base_assets so that effective_total_assets remains consistent.
    // We add `assets` to base_assets (not the full stream amount).
    {
