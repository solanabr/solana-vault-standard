...
//!
//! SVS-5: all functions use effective_total_assets(now) instead of
//! asset_vault.amount to reflect the streaming yield model.
...
pub struct VaultView<'info> {
    ...
    /// SVS-5: asset_vault kept for compatibility with deposit/withdraw CPIs,
    /// but total_assets is computed via effective_total_assets(now), not balance.
...
pub struct VaultViewWithOwner<'info> {
    ...
    /// SVS-5: asset_vault kept for compatibility; balance not used for share math
    #[account(constraint = asset_vault.key() == vault.asset_vault)]
    pub asset_vault: InterfaceAccount<'info, TokenAccount>,
...
/// Get effective total assets at current timestamp (streaming-aware).
pub fn get_total_assets(ctx: Context<VaultView>) -> Result<()> {
    // SVS-5: Return time-interpolated balance, not raw asset_vault.amount
    let clock = Clock::get()?;
...
pub fn max_withdraw(ctx: Context<VaultViewWithOwner>) -> Result<()> {
    ...
    // Calculate max assets owner can receive for their shares (floor - favors vault)
