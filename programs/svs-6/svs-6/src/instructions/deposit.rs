//! Deposit instruction: transfer assets, mint shares, deposit to confidential balance.
...
/// 2. Mint shares to user's non-confidential balance (vault PDA signs)
/// 3. Confidential deposit: move shares from non-confidential to pending balance
///
/// User must call apply_pending after this to make shares available.
/// User's shares account must be configured for CT (call configure_account first).
pub fn handler(ctx: Context<Deposit>, assets: u64, min_shares_out: u64) -> Result<()> {
    ...
    let shares = {
        ...
        // 1. Access control (whitelist/blacklist + frozen)
    ...
    // Step 4a: Transfer assets from user to vault
    transfer_checked(
        CpiContext::new(
    ...
    // Step 4b: Mint shares to user's NON-CONFIDENTIAL balance (vault PDA signs)
    ...
    // Step 4c: Move shares from non-confidential to confidential PENDING balance
    ...
    // ── 5. UPDATE STATE (streaming — arithmetic) ──
