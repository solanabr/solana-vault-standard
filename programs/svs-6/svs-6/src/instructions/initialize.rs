...
pub struct Initialize<'info> {
    ...
    #[account(
        init,
        payer = authority,
        space = ConfidentialStreamVault::LEN,
...
pub fn handler(
    ...
    // Step 1: Create shares mint account with space for extensions
    invoke_signed(
    ...
    // Step 2: Initialize ConfidentialTransferMint extension BEFORE mint initialization
    ...
    // Step 3: Initialize mint (vault PDA is mint authority, no freeze authority)
    ...
    // Streaming fields — no active stream at init
    vault.base_assets = 0;
    ...
    msg!(
        "Confidential streaming vault initialized: {} for asset {}",
        name,
        symbol
    );
