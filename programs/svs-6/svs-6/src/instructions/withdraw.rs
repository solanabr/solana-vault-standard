//! Withdraw instruction: CT inner_withdraw, burn shares, transfer assets.
...
/// to finalize accrued streaming yield and prevent stale-price mismatches.
#[derive(Accounts)]
...
pub fn handler(
    ...
    // ── 3. COMPUTE — ceiling rounding, user burns more shares ──
    ...
    // Convert decryptable balance bytes to PodAeCiphertext
    ...
    // Step 5a: CT inner_withdraw — moves shares from confidential to non-confidential balance
    ...
    // Step 5c: Transfer assets from vault to user (vault PDA signs)
    ...
    // ── 6. UPDATE STATE (streaming — arithmetic update) ──
