//! redeem_sol: burn exact shares to receive proportional native SOL.
//!
//! Flow:
...
/// Redeem exact shares for native SOL (floor rounding — protects vault).
pub fn handler(ctx: Context<RedeemSol>, shares: u64, min_lamports_out: u64) -> Result<()> {
    ...
    require!(net_assets >= min_lamports_out, VaultError::SlippageExceeded);
    ...
    // 6. Transfer wSOL from vault to user's wSOL account
    transfer_checked(
