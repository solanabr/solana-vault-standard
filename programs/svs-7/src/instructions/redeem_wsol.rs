//! Redeem wSOL instruction: burn exact shares, receive wSOL.
...
/// Redeem shares for wSOL (floor rounding — user gets less, protects vault).
/// Returns pre-wrapped wSOL for protocol composability.
pub fn handler(ctx: Context<RedeemWsol>, shares: u64, min_amount_out: u64) -> Result<()> {
    ...
    // Calculate assets (floor rounding)
    let amount = convert_to_assets(
        shares,
        total_assets,
        total_shares,
    ...
    let net_amount = {
        ...
        let result =
            module_hooks::apply_exit_fee(remaining, &crate::ID, &vault_key, amount)?;
        result.net_assets
    };
    ...
    // Slippage check
    require!(net_amount >= min_amount_out, VaultError::SlippageExceeded);
