//! Withdraw instructions: withdraw exact assets by burning required shares.
//! withdraw_sol: unwraps wSOL to native SOL for the user.
//! withdraw_wsol: transfers wSOL directly to user.
...
// =============================================================================
// withdraw_sol: Burn shares, unwrap wSOL, send native SOL to user
// =============================================================================
...
pub struct WithdrawSol<'info> {
    ...
    /// Temporary wSOL account to receive transfer then close (unwrap).
    /// Created by user, closed in this instruction to send native SOL to user.
    #[account(
        mut,
...
/// Withdraw exact lamports, burning required shares (ceiling rounding - protects vault).
/// Transfers wSOL from vault to temp account, then closes temp account to unwrap to native SOL.
pub fn withdraw_sol_handler(
    ...
    let net_assets = lamports;

    require!(
        lamports <= total_assets,
        SolVaultError::InsufficientAssets
    );
    ...
    require!(shares <= max_shares_in, SolVaultError::SlippageExceeded);
    require!(
    ...
    // The user is both the authority and destination of the close
...
// =============================================================================
// withdraw_wsol: Burn shares, transfer wSOL directly to user
// =============================================================================
...
/// Withdraw exact assets as wSOL, burning required shares (ceiling rounding - protects vault).
pub fn withdraw_wsol_handler(
    ...
    let net_assets = {
        ...
        let result = module_hooks::apply_exit_fee(remaining, &crate::ID, &vault_key, amount)?;
        result.net_assets
    };

    #[cfg(not(feature = "modules"))]
    let net_assets = amount;
