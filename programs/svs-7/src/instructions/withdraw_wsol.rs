//! withdraw_wsol: burn shares to withdraw exact wSOL (no unwrap).
//!
//! Standard SPL token withdraw — composability variant for protocols
//! that want to receive wSOL rather than native SOL.
...
pub fn handler(ctx: Context<WithdrawWsol>, lamports: u64, max_shares_in: u64) -> Result<()> {
    ...
    // 3. COMPUTE shares to burn (ceiling rounding — protects vault)
    ...
    // 5b. Transfer wSOL from vault to user (no close — user receives wSOL)
    transfer_checked(
