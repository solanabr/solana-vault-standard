//! SolVault state account definitions.

use anchor_lang::prelude::*;

use crate::constants::SOL_VAULT_SEED;

/// Balance model determines how total_assets is calculated.
#[derive(AnchorSerialize, AnchorDeserialize, Clone, Copy, PartialEq, Eq, Debug)]
pub enum BalanceModel {
    /// Live: reads wsol_vault.amount directly (like SVS-1)
    Live,
    /// Stored: uses vault.total_assets, requires sync() (like SVS-2)
    Stored,
}
...
pub struct SolVault {
    ...
    /// PDA-owned wSOL token account holding vault assets
    pub wsol_vault: Pubkey,
    /// Cached total assets in lamports (used by Stored model, unused by Live)
    pub total_assets: u64,
    /// Virtual offset exponent (9 - 9 = 0 for SOL)
    ...
    pub vault_id: u64,
    /// Balance model: Live reads wsol_vault.amount, Stored uses vault.total_assets
