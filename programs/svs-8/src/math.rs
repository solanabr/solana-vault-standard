...
pub fn total_portfolio_value(
    ...
    for i in 0..balances.len() {
        let num = (balances[i] as u128)
            ...
            .ok_or(VaultError::MathOverflow)?;
        let den = PRICE_SCALE
            .checked_mul(10u128.pow(asset_decimals[i] as u32))
            .ok_or(VaultError::MathOverflow)?;
        total = total
            .checked_add(num.checked_div(den).ok_or(VaultError::DivisionByZero)?)
            .ok_or(VaultError::MathOverflow)?;
    }
...
pub fn read_token_balance(info: &anchor_lang::prelude::AccountInfo) -> Result<u64> {
    ...
    let amount = u64::from_le_bytes(
        data[64..72]
            .try_into()
            .map_err(|_| anchor_lang::error!(crate::error::VaultError::MathOverflow))?,
    );
    Ok(amount)
}
