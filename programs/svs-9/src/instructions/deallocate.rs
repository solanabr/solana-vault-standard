...
pub fn deallocate_handler(ctx: Context<Deallocate>, shares_to_withdraw: u64, min_assets_out: u64) -> Result<()> {
    ...
    require!(shares_to_withdraw <= shares_before, VaultError::InsufficientShares);

    // Initial idle balance to calculate actual assets received
    let idle_before = ctx.accounts.idle_vault.amount;

    // 5. EXECUTE CPIs
    // Invoke child_vault::redeem(shares_to_withdraw, 0)
    ...
    let assets_received = current_idle.checked_sub(idle_before)
        .ok_or(VaultError::MathOverflow)?;

    let child_allocation = &mut ctx.accounts.child_allocation;
    let shares_after = shares_before.checked_sub(shares_to_withdraw)
        .ok_or(VaultError::MathOverflow)?;
