//! Module administration instructions for SVS-9.
//!
...
pub fn initialize_fee_config(
    ...
    fee_config.vault = ctx.accounts.allocator_vault.key();
    ...
    fee_config.high_water_mark = 10u64.pow(18); // SCALE from svs-fees logic
...
pub fn initialize_cap_config(
    ...
    cap_config.vault = ctx.accounts.allocator_vault.key();
...
pub fn initialize_lock_config(
    ...
    svs_locks::validate_lock_duration(lock_duration).map_err(|_| VaultError::MathOverflow)?;

    let lock_config = &mut ctx.accounts.lock_config;
    lock_config.vault = ctx.accounts.allocator_vault.key();
...
pub fn initialize_access_config(
    ...
    access_config.vault = ctx.accounts.allocator_vault.key();
