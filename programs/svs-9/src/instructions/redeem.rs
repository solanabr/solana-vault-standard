...
pub fn redeem_handler(ctx: Context<Redeem>, shares: u64, min_assets_out: u64) -> Result<()> {
    ...
    let assets = calculate_assets(shares, total_assets, total_shares, ctx.accounts.allocator_vault.decimals_offset)?;
    ...
    // Liquidity Rule
    require!(ctx.accounts.idle_vault.amount >= net_assets, VaultError::InsufficientAssets);
    ...
    // 6. UPDATE STATE
    ctx.accounts.allocator_vault.total_shares = total_shares.checked_sub(shares)
