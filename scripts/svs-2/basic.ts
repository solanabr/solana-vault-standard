/**
 * SVS-2 Basic Test Script
 *
 * Tests core vault functionality with stored balance model:
 * - Initialize vault
 * - Deposit assets
 * - Redeem shares
 * - Pause/unpause
 * - Sync stored balance
 *
 * Run: npx ts-node scripts/svs-2/basic.ts
 */
...
import { setupTest, getVaultPDA, getSharesMintPDA, explorerUrl, accountUrl, syncVault, ASSET_DECIMALS } from "./helpers";
...
async function main() {
  ...
  const initTx = await program.methods
    .initialize(vaultId, "SVS-2 Test Vault", "svVAULT2", "https://arweave.net/vault2-metadata")
    .accountsStrict({
  ...
  // Verify stored total_assets matches
  ...
  // Step 6: Sync (should be no-op since deposit updates stored total)
  console.log("\n" + "-".repeat(70));
  console.log("Step 6: Sync (no-op verification)");
  console.log("-".repeat(70));

  const syncTx = await syncVault(program, payer, vault, assetVault);
  console.log(`  Sync Tx: ${syncTx}`);

  const vaultAfterSync = await program.account.vault.fetch(vault);
  console.log(`  total_assets after sync: ${vaultAfterSync.totalAssets.toNumber() / 10 ** ASSET_DECIMALS}`);

  // Step 7: Redeem
  console.log("\n" + "-".repeat(70));
  console.log("Step 7: Redeeming shares");
  console.log("-".repeat(70));
