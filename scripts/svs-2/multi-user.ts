/**
 * SVS-2 Multi-User Fairness Test
 *
 * Run: npx ts-node scripts/svs-2/multi-user.ts
 */
...
async function main() {
  ...
  await program.methods
    .initialize(vaultId, "Multi-User Test", "MULTI2", "https://test.com")
    .accountsStrict({
  ...
  for (const user of users) {
    ...
    console.log(`\n  ${user.name} deposited ${user.initialDeposit.toLocaleString()} → ${user.sharesReceived.toLocaleString()} shares`);
  }
  ...
  for (const user of users) {
    ...
    console.log(`  ${user.name}: ${actualPct.toFixed(2)}% (expected ${expectedPct.toFixed(2)}%)`);
  }
