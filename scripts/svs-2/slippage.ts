/**
 * SVS-2 Slippage Protection Test
 *
 * Same slippage tests as SVS-1 but on the stored balance model.
 *
 * Run: npx ts-node scripts/svs-2/slippage.ts
 */
...
async function main() {
  ...
  await program.methods
    .initialize(vaultId, "Slippage Test Vault", "SLIP2", "https://test.com")
    .accountsStrict({
  ...
  console.log("-".repeat(70));
  try {
    await program.methods.deposit(new BN(1000 * 10 ** ASSET_DECIMALS), new BN(10_000_000 * 10 ** 9))
      .accountsStrict({
        user: payer.publicKey, vault, assetMint, userAssetAccount: userAta.address, assetVault, sharesMint, userSharesAccount,
        assetTokenProgram: TOKEN_PROGRAM_ID, token2022Program: TOKEN_2022_PROGRAM_ID,
        associatedTokenProgram: ASSOCIATED_TOKEN_PROGRAM_ID, systemProgram: SystemProgram.programId,
  ...
  console.log("-".repeat(70));
  try {
    await program.methods.deposit(new BN(1000 * 10 ** ASSET_DECIMALS), new BN(900 * 10 ** 9))
      .accountsStrict({
        user: payer.publicKey, vault, assetMint, userAssetAccount: userAta.address, assetVault, sharesMint, userSharesAccount,
        assetTokenProgram: TOKEN_PROGRAM_ID, token2022Program: TOKEN_2022_PROGRAM_ID,
        associatedTokenProgram: ASSOCIATED_TOKEN_PROGRAM_ID, systemProgram: SystemProgram.programId,
  ...
  console.log("-".repeat(70));
  try {
    await program.methods.mint(new BN(1000 * 10 ** 9), new BN(1 * 10 ** ASSET_DECIMALS))
      .accountsStrict({
        user: payer.publicKey, vault, assetMint, userAssetAccount: userAta.address, assetVault, sharesMint, userSharesAccount,
        assetTokenProgram: TOKEN_PROGRAM_ID, token2022Program: TOKEN_2022_PROGRAM_ID,
        associatedTokenProgram: ASSOCIATED_TOKEN_PROGRAM_ID, systemProgram: SystemProgram.programId,
  ...
  console.log("-".repeat(70));
  try {
    await program.methods.withdraw(new BN(1000 * 10 ** ASSET_DECIMALS), new BN(1 * 10 ** 9))
      .accountsStrict({
        user: payer.publicKey, vault, assetMint, userAssetAccount: userAta.address, assetVault, sharesMint, userSharesAccount,
        assetTokenProgram: TOKEN_PROGRAM_ID, token2022Program: TOKEN_2022_PROGRAM_ID,
      }).rpc();
  ...
  try {
    await program.methods.redeem(sharesToRedeem, new BN(1_000_000 * 10 ** ASSET_DECIMALS))
      .accountsStrict({
        user: payer.publicKey, vault, assetMint, userAssetAccount: userAta.address, assetVault, sharesMint, userSharesAccount,
        assetTokenProgram: TOKEN_PROGRAM_ID, token2022Program: TOKEN_2022_PROGRAM_ID,
      }).rpc();
  ...
  console.log("-".repeat(70));
  try {
    await program.methods.redeem(sharesToRedeem, new BN(100 * 10 ** ASSET_DECIMALS))
      .accountsStrict({
        user: payer.publicKey, vault, assetMint, userAssetAccount: userAta.address, assetVault, sharesMint, userSharesAccount,
        assetTokenProgram: TOKEN_PROGRAM_ID, token2022Program: TOKEN_2022_PROGRAM_ID,
      }).rpc();
