/**
 * SVS-5 Basic Test Script
 *
 * Tests core streaming vault functionality:
 * - Initialize vault
 * - Deposit assets
 * - Distribute yield as stream
 * - Checkpoint accrued yield
 * - Redeem shares with profit
 * - Pause/unpause
 *
 * Run: npx ts-node scripts/svs-5/basic.ts
 */
...
async function main() {
  ...
  console.log("\n" + "-".repeat(70));
  console.log("Step 4: Initializing streaming vault");
  console.log("-".repeat(70));
  ...
  // Step 6: Distribute Yield
  console.log("\n" + "-".repeat(70));
  console.log("Step 6: Distributing yield as stream");
  console.log("-".repeat(70));

  const yieldAmount = new BN(YIELD_AMOUNT * 10 ** ASSET_DECIMALS);
  const yieldTx = await program.methods
    .distributeYield(yieldAmount, new BN(STREAM_DURATION))
    .accountsStrict({
      authority: payer.publicKey,
      vault, assetMint,
  ...
  console.log(`  Tx: ${yieldTx}`);

  let vaultAccount = await program.account.streamVault.fetch(vault);
  console.log(`  Stream Amount: ${vaultAccount.streamAmount.toString()}`);
  console.log(`  Stream Start: ${vaultAccount.streamStart.toString()}`);
  console.log(`  Stream End: ${vaultAccount.streamEnd.toString()}`);

  // Step 7: Checkpoint
  console.log("\n" + "-".repeat(70));
  console.log("Step 7: Running checkpoint (materializing accrued yield)");
  console.log("-".repeat(70));

  // Wait briefly for some yield to accrue
  console.log("  Waiting 5s for yield to accrue...");
  await new Promise(r => setTimeout(r, 5000));

  const checkpointTx = await program.methods
    .checkpoint()
    .accountsStrict({ vault })
    .rpc();

  vaultAccount = await program.account.streamVault.fetch(vault);
  console.log(`  Tx: ${checkpointTx}`);
  console.log(`  Base Assets After Checkpoint: ${vaultAccount.baseAssets.toString()}`);
  console.log(`  Remaining Stream: ${vaultAccount.streamAmount.toString()}`);

  // Step 8: Redeem
  console.log("\n" + "-".repeat(70));
  console.log("Step 8: Redeeming shares");
  console.log("-".repeat(70));

  const redeemShares = new BN(Math.floor(Number(userSharesAfterDeposit.amount) / 2));
