/**
 * SVS-5 Slippage Protection Test
 *
 * Tests that min/max slippage parameters work correctly during active streams:
 ...
 *
 * Run: npx ts-node scripts/svs-5/slippage.ts
 */
...
async function main() {
  ...
  await program.methods
    .distributeYield(new BN(1_000 * 10 ** ASSET_DECIMALS), new BN(120))
    .accountsStrict({
  ...
  console.log("  Setup complete (yield stream active)\n");
