...
async function main() {
  const { connection, payer, program, programId } = await setupSvs7Test("Withdraw & Mint (SOL)");
  ...
  console.log("TEST 1: mint_sol happy path (1 SOL worth of shares, generous cap)");
  console.log("-".repeat(70));
  ...
  try {
    ...
    console.log(`  SOL spent: ${lamportsSpent / LAMPORTS_PER_SOL} SOL (includes fees)`);
    console.log(`  Explorer: ${explorerUrl(mintTx)}`);

    if (sharesGained > 0 && lamportsSpent > 0 && lamportsSpent <= maxLamportsIn.toNumber() + 5000) {
  ...
  // ============================================================================
  console.log("\n" + "-".repeat(70));
  console.log("TEST 2: mint_sol with maxLamportsIn too low (slippage protection)");
  console.log("-".repeat(70));
  ...
  // ============================================================================
  console.log("\n" + "-".repeat(70));
  console.log("TEST 4: withdraw_sol with maxSharesIn too low (slippage protection)");
  console.log("-".repeat(70));
  ...
  const sharesBefore5 = Number((await getAccount(connection, userSharesAccount, undefined, TOKEN_2022_PROGRAM_ID)).amount);
  ...
  const wsolBefore5 = await getAccount(connection, userWsolAccount, undefined, TOKEN_PROGRAM_ID);

  try {
    const withdrawWsolTx = await program.methods
      .withdrawWsol(new BN(Math.floor(0.5 * LAMPORTS_PER_SOL)), new BN(2 * LAMPORTS_PER_SOL))
      .accountsStrict({
        user: payer.publicKey,
        vault,
        nativeMint: NATIVE_MINT,
    ...
    const sharesAfter5 = Number((await getAccount(connection, userSharesAccount, undefined, TOKEN_2022_PROGRAM_ID)).amount);
    const wsolAfter5 = await getAccount(connection, userWsolAccount, undefined, TOKEN_PROGRAM_ID);

    const sharesBurned5 = sharesBefore5 - sharesAfter5;
    const wsolReceived5 = Number(wsolAfter5.amount) - Number(wsolBefore5.amount);
  ...
  console.log(`  Total assets (wSOL vault): ${totalAssets / LAMPORTS_PER_SOL} SOL`);
  ...
  if (totalAssets > 0 && totalShares > 0) {
    ...
    console.log("  PASSED: Consistent state"); passed++;
  } else {
  ...
  console.log(`  mint_sol/withdraw_sol/withdraw_wsol ${failed === 0 ? "ALL WORKING" : "HAS ISSUES"}`);
  console.log("=".repeat(70) + "\n");
