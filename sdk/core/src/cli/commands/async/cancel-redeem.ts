...
export function registerCancelRedeemCommand(program: Command): void {
  program
    ...
    .description("Cancel a pending async redeem request")
    ...
    .action(async (vaultArg, opts) => {
      ...
      try {
        ...
        output.info("Cancelling redeem request");
        ...
        if (globalOpts.output === "json") {
          output.json({ success: true, signature: sig, vault: vaultArg, operation: "cancel-redeem" });
        }
      } catch (error) {
        output.error(`Cancel redeem failed: ${error instanceof Error ? error.message : String(error)}`);
        process.exit(1);
      }
    });
}
