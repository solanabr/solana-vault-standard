...
export function registerFulfillDepositCommand(program: Command): void {
  program
    ...
    .requiredOption("--owner <pubkey>", "Owner of the deposit request")
    ...
    .action(async (vaultArg, opts) => {
      ...
      try {
        ...
        output.info(`Fulfilling deposit for owner: ${owner.toBase58()}`);
        ...
        output.success(`Deposit fulfilled for ${owner.toBase58()}`);
        output.info(`Signature: ${sig}`);

        if (globalOpts.output === "json") {
          output.json({ success: true, signature: sig, vault: vaultArg, operation: "fulfill-deposit", owner: owner.toBase58() });
        }
      } catch (error) {
        output.error(`Fulfill deposit failed: ${error instanceof Error ? error.message : String(error)}`);
        process.exit(1);
      }
    });
}
