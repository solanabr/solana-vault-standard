...
export function registerRejectDepositCommand(program: Command): void {
  program
    ...
    .action(async (vaultArg, opts) => {
      ...
      try {
        ...
        output.info(`Rejecting deposit for investor: ${investor.toBase58()}`);
        ...
        const sig = await vault.rejectDeposit(wallet.publicKey, investor);

        spinner.succeed("Transaction confirmed");
        output.success(`Deposit rejected for ${investor.toBase58()}`);
