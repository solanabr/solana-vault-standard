...
describe("SVS-9 E2E CPI CPI Test", () => {
  ...
  before(async () => {
    ...
    childVaultClient = await SolanaVault.create(svs1Program as unknown as Program, {
      assetMint,
    ...
    allocatorClient = await AllocatorVaultClient.create(
      svs9Program as unknown as Program,
      {
        vaultId: allocatorVaultId,
        idleBufferBps: 1000,
        assetMint,
        curator: payer.publicKey,
      },
    );
    allocatorSharesMint = (await allocatorClient.getState()).sharesMint;
  ...
  it("6. Deallocates principal back to idle liquidity", async () => {
    ...
    await allocatorClient.deallocate({
      sharesToWithdraw,
      minAssetsOut: new BN(0),
      childVault: childVaultClient.vault,
      childProgram: svs1Program.programId,
    ...
    expect(idleAfter.toNumber()).to.be.greaterThan(idleBefore.toNumber());
    expect(allocationAfter.depositedAssets.toNumber()).to.be.lessThan(
      allocationBefore.depositedAssets.toNumber()
    );
    expect(Number(childSharesAfter.amount)).to.be.lessThan(
      Number(childSharesBefore.amount)
    );
  });
});
