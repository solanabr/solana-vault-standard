  ...
  async function getRemainingAccounts() {
    ...
    for (const alloc of childAllocations) {
      if (alloc.account.enabled) {
        ...
        if (!childVaultInfo) {
          throw new Error("Child vault account not found");
        }

        let childAssetVault: PublicKey;
        let childSharesMint: PublicKey;
        if (
          childVaultInfo.data.length === 197 ||
          childVaultInfo.data.length === 201 ||
          childVaultInfo.data.length === 211
        ) {
          childSharesMint = new PublicKey(childVaultInfo.data.subarray(72, 104));
          childAssetVault = new PublicKey(childVaultInfo.data.subarray(104, 136));
        } else if (
          childVaultInfo.data.length === 246 ||
          childVaultInfo.data.length === 254
        ) {
          childSharesMint = new PublicKey(childVaultInfo.data.subarray(72, 104));
          childAssetVault = new PublicKey(childVaultInfo.data.subarray(104, 136));
        } else {
          throw new Error(
            `Unsupported child vault layout: ${childVaultInfo.data.length}`
          );
        }
